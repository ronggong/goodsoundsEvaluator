...
from keras.layers import Activation, Dropout, Dense, Reshape, Flatten, merge, ELU, GRU, Permute, Masking, BatchNormalization, Input
...
def crnn_model(filter_density, pool_n_row, pool_n_col, nlen, dropout, include_top=True):
    ...
    x = Convolution2D(int(14 * filter_density), 3, 3, border_mode='valid',
    ...
    x_res_1_left = Convolution2D(int(28 * filter_density), 3, 3, border_mode='same',
                  input_shape=reshape_dim, dim_ordering='tf',
                  init='he_uniform', W_regularizer=l2(1e-5))(x)
    ...
    x_res_1_right = Convolution2D(int(28 * filter_density), 3, 3, border_mode='same',
                                 input_shape=reshape_dim, dim_ordering='tf',
                                 init='he_uniform', W_regularizer=l2(1e-5))(x)
    ...
    x_res_2_left = Convolution2D(int(28 * filter_density), 3, 3, border_mode='same',
                                 input_shape=reshape_dim, dim_ordering='tf',
    ...
    x = AveragePooling2D(pool_size=(4, 26), border_mode='valid', dim_ordering='tf')(x)

    x = Dropout(0.1)(x)

    # print(x.shape[0])
    #
    # x = Reshape((155 * 1, 28))(x)
    x = Flatten()(x)
    #
...
if __name__ == '__main__':
    ...
    # nb_epoch = 10
    file_path_model = '/scratch/rgongcrnnSingleSoundEvaluator_'+feature_string+'_gru1layer_10nodes_dense/out/crnn_model_cw_'+feature_string+'_gru1layer_32nodes_resnet_35k_nogru.h5'
