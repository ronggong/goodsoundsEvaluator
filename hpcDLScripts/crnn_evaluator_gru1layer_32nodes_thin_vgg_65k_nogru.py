...
def crnn_model(filter_density, pool_n_row, pool_n_col, nlen, dropout, include_top=True):
    ...
    model.add(Convolution2D(int(17*filter_density), 3, 3, border_mode='valid',
    ...
    # conv 2
    model.add(Convolution2D(int(34 * filter_density), 3, 3, border_mode='valid',
                            input_shape=reshape_dim, dim_ordering='tf',
                            init='he_uniform', W_regularizer=l2(1e-5)))
    ...
    # conv 4
    model.add(Convolution2D(int(51 * filter_density), 3, 3, border_mode='valid',
                            input_shape=reshape_dim, dim_ordering='tf',
                            init='he_uniform', W_regularizer=l2(1e-5)))
    # model.add(BatchNormalization(axis=channel_axis, mode=0, name='bn3'))
    ...
    model.add(MaxPooling2D(pool_size=(2, 3), border_mode='valid', dim_ordering='tf'))
    model.add(Dropout(0.1))
    ...
    # model.add(Reshape((model.output_shape[1]*model.output_shape[2],51)))
    model.add(Flatten())
...
if __name__ == '__main__':
    ...
    file_path_model = '/scratch/rgongcrnnSingleSoundEvaluator_'+feature_string+'_gru1layer_10nodes_dense/out/crnn_model_cw_'+feature_string+'_gru1layer_32nodes_thin_vgg_65k_nogru.h5'
