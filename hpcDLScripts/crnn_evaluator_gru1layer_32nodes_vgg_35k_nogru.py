...
def crnn_model(filter_density, pool_n_row, pool_n_col, nlen, dropout, include_top=True):
    ...
    # conv 1
    model.add(Convolution2D(int(45*filter_density), 3, 3, border_mode='valid',
                            input_shape=reshape_dim, dim_ordering='tf',
                            init='he_uniform', W_regularizer=l2(1e-5)))
    ...
    model.add(MaxPooling2D(pool_size=(3, 2), border_mode='valid', dim_ordering='tf'))
    model.add(Dropout(0.1))
    ...
    # model.add(Reshape((model.output_shape[1]*model.output_shape[2],45)))
    model.add(Flatten())
...
if __name__ == '__main__':
    ...
    # nb_epoch = 10
    file_path_model = '/scratch/rgongcrnnSingleSoundEvaluator_'+feature_string+'_gru1layer_10nodes_dense/out/crnn_model_cw_'+feature_string+'_gru1layer_32nodes_fat_vgg_35k_nogru.h5'
