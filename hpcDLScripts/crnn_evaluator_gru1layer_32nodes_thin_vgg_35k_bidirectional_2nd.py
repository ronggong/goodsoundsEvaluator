...
def loadDataPath(feature_string):
    ...
    filename_all_set = '/scratch/rgongcrnnSingleSoundEvaluator_' + feature_string + '_gru1layer_thin_bidi/data/allData.pkl'
    filename_scaler = '/scratch/rgongcrnnSingleSoundEvaluator_' + feature_string + '_gru1layer_thin_bidi/data/scaler_' + feature_string + '_train.pkl'
    filename_trainIndex = '/scratch/rgongcrnnSingleSoundEvaluator_'+feature_string+'_gru1layer_thin_bidi/data/trainIndex_'+feature_string+'.pkl'
...
def loadData(feature_string, filename_all_set, filename_scaler, filename_trainIndex):
    ...
    #
    # filename_train_validation_set = '/scratch/rgongcrnnSingleSoundEvaluator_'+feature_string+'_gru1layer_thin_bidi/data/allData.pkl'
...
def crnn_model(filter_density, pool_n_row, pool_n_col, nlen, dropout, include_top=True):
    ...
    # conv 2
    model.add(Convolution2D(int(22 * filter_density), 3, 3, border_mode='valid',
                            input_shape=reshape_dim, dim_ordering='tf',
                            init='he_uniform', W_regularizer=l2(1e-5)))
    ...
    model.add(Reshape((model.output_shape[1]*model.output_shape[2],32)))
    # model.add(Flatten())
    ...
    # model.add(Dense(output_dim=128, init='he_uniform', W_regularizer=l2(1e-5)))
    # model.add(ELU())
    # model.add(Dropout(0.3))
...
def train_model(filter_density,
    ...
    save_clb = ModelCheckpoint("{model_basepath}/".format(model_basepath=file_path_model) +
                               "{model_filename}".format(model_filename=file_name_model) +
                               "epoch.{epoch:02d}-loss.{loss:.3f}.h5",
                               monitor='loss',
                               save_best_only=True)
...
if __name__ == '__main__':
    ...
    # nb_epoch = 48
    file_path_model = '/scratch/rgongcrnnSingleSoundEvaluator_'+feature_string+'_gru1layer_thin_bidi/out/'
    # file_path_model = '../models/'
    file_name_model = 'crnn_model_cw_'+feature_string+'_gru1layer_32nodes_thin_vgg_35k_bidirectional'
    ...
    train_model(filter_density=1,
                ...
                file_name_model=file_name_model,
                X_train=X_train,
                X_val=X_val,
